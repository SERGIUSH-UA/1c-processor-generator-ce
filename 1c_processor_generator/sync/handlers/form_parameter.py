"""
Handler for Form Parameters.

Form parameters are form-level inputs passed when opening a form.
v2.36.0+ feature.
"""
...
class FormParameterHandler(BaseElementHandler):
    """
    Handler for Form Parameter sync operations.

    YAML location: forms[].parameters[]
    XML location: Form.xml //form:Parameters/form:Parameter

    Form parameters are used to:
    - Pass data when opening a form (filters, default values)
    - Create wizard/dialog interfaces
    ...
    def extract_from_xml(
        ...
        """Extract form parameter data from XML."""
        ...
        if type_elem is not None and type_elem.text:
            ...
            # Extract type name from xs:string, xs:boolean, etc.
            if ':' in type_text:
    ...
    def get_elements_from_tree(
        ...
        """Get form parameters from Form.xml."""
        ns = namespaces or self.NAMESPACES
        parameters = {}
    ...
    def add_to_yaml(
        ...
        # Restructure synonym if present (convert flat to nested)
    ...
    def delete_from_yaml(
        ...
        """Delete form parameter from YAML."""
        if 'forms' not in config or form_index >= len(config['forms']):
            return False
    ...
    def check_references(
        ...
        """Check for references to form parameter in BSL."""
        references = []

        # Check BSL code for parameter usage
        patterns = [
            f"Параметры.{name}",
            f"Parameters.{name}",
            f'"{name}"',
    ...
    def _get_type(
        ...
        """Get parameter type from XML element."""
        type_elem = elem.find(".//v8:Type", namespaces=ns)
        if type_elem is not None and type_elem.text:
            type_text = type_elem.text
            if ':' in type_text:
    ...
    def _get_key_parameter(self, elem: "etree._Element") -> bool:
        """Get key_parameter value from XML element."""
