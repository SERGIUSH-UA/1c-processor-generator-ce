"""
Handler for Form Attributes.

Form attributes are form-level data items (SpreadsheetDocument, BinaryData, etc.).
"""
...
class FormAttributeHandler(BaseElementHandler):
    """
    Handler for form attribute sync operations.

    YAML location: forms[].form_attributes[]
    XML location: Form.xml //form:Attributes/form:Attribute (non-ValueTable types)
    """
    ...
    def extract_from_xml(
        ...
        """Extract form attribute data from XML."""
    ...
    def get_elements_from_tree(
        ...
        """Get form attributes from Form.xml."""
        ns = namespaces or self.NAMESPACES
        attrs = {}
        ...
        # Form attributes that are NOT ValueTable
        for attr in form_root.xpath(".//form:Attribute", namespaces=ns):
            ...
            if not is_value_table:
                name = attr.get("name")
                # Skip special system attributes
    ...
    def delete_from_yaml(
        ...
        """Delete form attribute from YAML."""
        if 'forms' not in config or form_index >= len(config['forms']):
            return False
    ...
    def check_references(
        ...
        """Check for references to form attribute in BSL."""
        references = []
