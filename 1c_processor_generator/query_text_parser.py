"""
Парсер query_text для DynamicList - витягує поля з SELECT для генерації заглушок.

Підтримує:
- Парсинг SELECT полів з query_text
- Парсинг DynamicListColumn.field (коли немає query_text)
- Визначення типів з контексту (Date, Number, Ref)
- Фільтрацію стандартних атрибутів (Дата, Номер, Ссылка тощо)
"""

import re
import logging
from dataclasses import dataclass, field
from typing import List, Optional, Dict, Set, Tuple

from .models import generate_uuid, DynamicListColumn

logger = logging.getLogger(__name__)


@dataclass
class StubAttribute:
    """Attribute for stub Document/Catalog generation."""
    name: str
    type: str  # "string", "number", "date", "boolean", "CatalogRef.*", "DocumentRef.*"
    length: int = 0  # For string (0 = unlimited)
    digits: int = 15  # For number
    fraction_digits: int = 2  # For number
    uuid: str = field(default_factory=generate_uuid)


class QueryTextParser:
    """
    Parser for 1C SQL query_text and DynamicList columns.

    Extracts field names from SELECT clause and determines their types
    for stub metadata generation.
    """

    # Standard attributes that DON'T need generation (built-in to Document)
    STANDARD_DOCUMENT_ATTRS: Set[str] = {
        # Russian
        'Ссылка', 'Дата', 'Номер', 'Проведен', 'ПометкаУдаления',
        # English
        'Ref', 'Date', 'Number', 'Posted', 'DeletionMark',
    }

    # Standard attributes that DON'T need generation (built-in to Catalog)
    STANDARD_CATALOG_ATTRS: Set[str] = {
        # Russian
        'Ссылка', 'Код', 'Наименование', 'ПометкаУдаления',
        'Родитель', 'Владелец', 'Предопределенный', 'ЭтоГруппа',
        # English
        'Ref', 'Code', 'Description', 'DeletionMark',
        'Parent', 'Owner', 'Predefined', 'IsFolder',
    }

    # Patterns for type inference by field name
    DATE_PATTERNS = [
        r'.*Дата.*', r'.*Date.*', r'.*Период.*', r'.*Period.*',
    ]

    NUMBER_PATTERNS = [
        r'.*Сумма.*', r'.*Количество.*', r'.*Курс.*', r'.*Кратность.*',
        r'.*Amount.*', r'.*Quantity.*', r'.*Rate.*', r'.*Price.*',
        r'.*Цена.*', r'.*Стоимость.*', r'.*Cost.*',
    ]

    BOOLEAN_PATTERNS = [
        r'.*Проведен.*', r'.*Posted.*', r'.*ПометкаУдаления.*',
        r'.*DeletionMark.*', r'.*ЭтоГруппа.*', r'.*IsFolder.*',
    ]

    def __init__(self):
        """Initialize parser with compiled regex patterns."""
        self._date_patterns = [re.compile(p, re.IGNORECASE) for p in self.DATE_PATTERNS]
        self._number_patterns = [re.compile(p, re.IGNORECASE) for p in self.NUMBER_PATTERNS]
        self._boolean_patterns = [re.compile(p, re.IGNORECASE) for p in self.BOOLEAN_PATTERNS]

    def extract_stub_attributes(
        self,
        query_text: str,
        main_table: Optional[str] = None
    ) -> List[StubAttribute]:
        """
        Extract attributes from query_text for stub generation.

        Args:
            query_text: 1C SQL query (ВЫБРАТЬ ... ИЗ ...)
            main_table: Optional main_table like "Document.РеализацияТоваровУслуг"

        Returns:
            List of StubAttribute objects for non-standard fields
        """
        if not query_text:
            return []

        # Determine metadata type from main_table
        metadata_type = self._get_metadata_type(main_table)
        standard_attrs = (
            self.STANDARD_DOCUMENT_ATTRS if metadata_type == "Document"
            else self.STANDARD_CATALOG_ATTRS
        )

        # Parse FROM clause for table aliases
        table_aliases = self._parse_from_clause(query_text)

        # Parse SELECT clause for fields
        fields = self._parse_select_clause(query_text)

        # Convert fields to StubAttributes (skip standard)
        attributes = []
        seen_names: Set[str] = set()

        for field_name, source_table, source_field in fields:
            # Skip standard attributes
            if field_name in standard_attrs or source_field in standard_attrs:
                logger.debug(f"Skipping standard attribute: {field_name}")
                continue

            # Skip duplicates
            if field_name in seen_names:
                continue
            seen_names.add(field_name)

            # Infer type
            attr_type = self._infer_type(field_name, source_field, source_table, table_aliases)

            attr = StubAttribute(
                name=field_name,
                type=attr_type,
                length=0 if attr_type == "string" else 0,
                digits=15 if attr_type == "number" else 0,
                fraction_digits=2 if attr_type == "number" else 0,
            )
            attributes.append(attr)
            logger.debug(f"Extracted attribute: {field_name} -> {attr_type}")

        return attributes

    def extract_from_columns(
        self,
        columns: List[DynamicListColumn],
        main_table: str
    ) -> List[StubAttribute]:
        """
        Extract attributes from DynamicList columns when no query_text.

        Used when manual_query=False and columns are defined.

        Args:
            columns: List of DynamicListColumn objects
            main_table: Main table like "Document.РеализацияТоваровУслуг"

        Returns:
            List of StubAttribute objects for non-standard fields
        """
        if not columns or not main_table:
            return []

        # Determine metadata type from main_table
        metadata_type = self._get_metadata_type(main_table)
        standard_attrs = (
            self.STANDARD_DOCUMENT_ATTRS if metadata_type == "Document"
            else self.STANDARD_CATALOG_ATTRS
        )

        attributes = []
        seen_names: Set[str] = set()

        for col in columns:
            field_name = col.field

            # Skip standard attributes
            if field_name in standard_attrs:
                logger.debug(f"Skipping standard attribute: {field_name}")
                continue

            # Skip duplicates
            if field_name in seen_names:
                continue
            seen_names.add(field_name)

            # Infer type from field name only
            attr_type = self._infer_type_from_name(field_name)

            attr = StubAttribute(
                name=field_name,
                type=attr_type,
                length=0 if attr_type == "string" else 0,
                digits=15 if attr_type == "number" else 0,
                fraction_digits=2 if attr_type == "number" else 0,
            )
            attributes.append(attr)
            logger.debug(f"Extracted from column: {field_name} -> {attr_type}")

        return attributes

    def _get_metadata_type(self, main_table: Optional[str]) -> str:
        """Get metadata type (Document/Catalog) from main_table string."""
        if not main_table:
            return "Document"  # Default

        main_table_lower = main_table.lower()
        if main_table_lower.startswith("document.") or main_table_lower.startswith("документ."):
            return "Document"
        elif main_table_lower.startswith("catalog.") or main_table_lower.startswith("справочник."):
            return "Catalog"
        else:
            return "Document"  # Default

    def _parse_from_clause(self, query_text: str) -> Dict[str, Tuple[str, str]]:
        """
        Parse FROM clause to extract table aliases.

        Returns dict: alias -> (metadata_type, metadata_name)
        Example: {"Док": ("Document", "РеализацияТоваровУслуг")}
        """
        aliases: Dict[str, Tuple[str, str]] = {}

        # Pattern: ИЗ Документ.ИмяДокумента КАК Алиас
        # Also: FROM Document.Name AS Alias
        patterns = [
            # Russian: Документ
            r'(?:ИЗ|FROM)\s+Документ\.(\w+)\s+(?:КАК|AS)\s+(\w+)',
            # Russian: Справочник
            r'(?:ИЗ|FROM)\s+Справочник\.(\w+)\s+(?:КАК|AS)\s+(\w+)',
            # English: Document
            r'(?:ИЗ|FROM)\s+Document\.(\w+)\s+(?:КАК|AS)\s+(\w+)',
            # English: Catalog
            r'(?:ИЗ|FROM)\s+Catalog\.(\w+)\s+(?:КАК|AS)\s+(\w+)',
        ]

        metadata_types = ["Document", "Catalog", "Document", "Catalog"]

        for pattern, meta_type in zip(patterns, metadata_types):
            for match in re.finditer(pattern, query_text, re.IGNORECASE):
                name, alias = match.groups()
                aliases[alias] = (meta_type, name)
                logger.debug(f"Found table alias: {alias} -> {meta_type}.{name}")

        # Also try patterns without alias (just table name)
        patterns_no_alias = [
            r'(?:ИЗ|FROM)\s+Документ\.(\w+)(?:\s|$|,)',
            r'(?:ИЗ|FROM)\s+Справочник\.(\w+)(?:\s|$|,)',
            r'(?:ИЗ|FROM)\s+Document\.(\w+)(?:\s|$|,)',
            r'(?:ИЗ|FROM)\s+Catalog\.(\w+)(?:\s|$|,)',
        ]

        for pattern, meta_type in zip(patterns_no_alias, metadata_types):
            for match in re.finditer(pattern, query_text, re.IGNORECASE):
                name = match.group(1)
                # Use full table name as alias
                aliases[name] = (meta_type, name)

        return aliases

    def _parse_select_clause(self, query_text: str) -> List[Tuple[str, Optional[str], str]]:
        """
        Parse SELECT clause to extract fields.

        Returns list of tuples: (field_alias, source_table_alias, source_field)
        """
        fields: List[Tuple[str, Optional[str], str]] = []

        # Extract SELECT ... FROM part
        # Note: We need to match standalone ИЗ/FROM (with newline before or start of line)
        # to avoid matching "ИЗ" inside field names like "РеализацИЗацияТоваровУслуг"
        select_match = re.search(
            r'(?:ВЫБРАТЬ|SELECT)\s+(.+?)\n\s*(?:ИЗ|FROM)\s+',
            query_text,
            re.IGNORECASE | re.DOTALL
        )

        if not select_match:
            # Fallback: try simpler pattern for single-line queries
            select_match = re.search(
                r'(?:ВЫБРАТЬ|SELECT)\s+(.+?)\s+(?:ИЗ|FROM)\s+\w+\.',
                query_text,
                re.IGNORECASE | re.DOTALL
            )

        if not select_match:
            logger.warning("Could not find SELECT ... FROM in query_text")
            return fields

        select_part = select_match.group(1)

        # Skip literals: "текст" КАК Алиас
        literal_pattern = r'"[^"]*"\s+(?:КАК|AS)\s+\w+'
        select_part = re.sub(literal_pattern, '', select_part)

        # Pattern 1: ТаблицаАлиас.Поле КАК Алиас
        pattern_with_alias = r'(\w+)\.(\w+)\s+(?:КАК|AS)\s+(\w+)'
        for match in re.finditer(pattern_with_alias, select_part, re.IGNORECASE):
            table_alias, source_field, field_alias = match.groups()
            fields.append((field_alias, table_alias, source_field))

        # Pattern 2: ТаблицаАлиас.Поле (without alias)
        # Need to avoid matching fields already matched above
        already_matched = set()
        for f in fields:
            already_matched.add(f[2])  # source_field

        pattern_no_alias = r'(\w+)\.(\w+)(?=\s*,|\s*$|\s+(?:ИЗ|FROM))'
        for match in re.finditer(pattern_no_alias, select_part, re.IGNORECASE):
            table_alias, source_field = match.groups()
            # Check this wasn't already matched with an alias
            check_str = f"{table_alias}.{source_field}"
            if check_str not in [f"{f[1]}.{f[2]}" for f in fields]:
                fields.append((source_field, table_alias, source_field))

        return fields

    def _infer_type(
        self,
        field_name: str,
        source_field: str,
        source_table: Optional[str],
        table_aliases: Dict[str, Tuple[str, str]]
    ) -> str:
        """
        Infer type for a field based on name and context.

        Priority:
        1. Pattern matching on field name
        2. Reference type from table aliases
        3. Default to string
        """
        # Try to determine from field name patterns
        name_type = self._infer_type_from_name(field_name)
        if name_type != "string":
            return name_type

        # Also check source field name
        source_type = self._infer_type_from_name(source_field)
        if source_type != "string":
            return source_type

        # Check if it's a reference field (ends with common ref patterns)
        # For stub generation, use string type for ref-like fields
        # because we don't know the actual catalog type
        ref_patterns = [
            r'.*Организация$', r'.*Organization$',
            r'.*Контрагент$', r'.*Counterparty$', r'.*Partner$',
            r'.*Номенклатура$', r'.*Product$', r'.*Item$',
            r'.*Валюта$', r'.*Currency$',
            r'.*Склад$', r'.*Warehouse$',
            r'.*Сотрудник$', r'.*Employee$',
            r'.*Пользователь$', r'.*User$',
            r'.*Подразделение$', r'.*Department$',
            r'.*Договор$', r'.*Contract$',
        ]

        for pattern in ref_patterns:
            if re.match(pattern, field_name, re.IGNORECASE) or re.match(pattern, source_field, re.IGNORECASE):
                # It's likely a CatalogRef, but for stub generation we use string
                # The actual type doesn't matter for DataPath validation
                return "string"

        return "string"

    def _infer_type_from_name(self, field_name: str) -> str:
        """Infer type from field name patterns only."""
        # Check date patterns
        for pattern in self._date_patterns:
            if pattern.match(field_name):
                return "date"

        # Check number patterns
        for pattern in self._number_patterns:
            if pattern.match(field_name):
                return "number"

        # Check boolean patterns
        for pattern in self._boolean_patterns:
            if pattern.match(field_name):
                return "boolean"

        return "string"


def extract_stub_attributes_for_dynamic_list(
    query_text: Optional[str],
    columns: Optional[List[DynamicListColumn]],
    main_table: Optional[str],
) -> List[StubAttribute]:
    """
    Convenience function to extract stub attributes from DynamicList.

    Tries query_text first, falls back to columns.
    """
    parser = QueryTextParser()

    # Try query_text first
    if query_text:
        attrs = parser.extract_stub_attributes(query_text, main_table)
        if attrs:
            return attrs

    # Fallback to columns
    if columns:
        return parser.extract_from_columns(columns, main_table)

    return []
