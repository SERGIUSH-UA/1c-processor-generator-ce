"""
CLI інтерфейс для генератора зовнішніх обробок 1C

Використання:
    python -m 1c_processor_generator minimal НазваОбробки
...
try:
    ...
    from .pro.licensed_compiler import LicensedEPFCompiler
    from .pro.license import get_license_manager
    from .pro.version_checker import check_version_in_background
except ImportError:
    # Якщо запускається напряму (python __main__.py)
    from generator import ProcessorGenerator, create_minimal_processor
    from models import Processor, ValidationConfig
    from metadata_analyzer import MetadataAnalyzer
    from yaml_parser import parse_yaml_config
...
def create_example_processor() -> Processor:
    """Створює приклад обробки з табличною частиною"""
    processor = Processor(
...
def cmd_minimal(args):
    """Команда: створити мінімальну обробку"""
...
def cmd_example(args):
    """Команда: створити приклад обробки"""
...
def _cloud_compile(args):
    """v2.75.0: Cloud EPF compilation via gen.itdeo.tech."""
    print(f"☁️  Хмарна компіляція EPF: {args.config}...")

    # Check PRO license with cloud_compilation feature
    ...
    # Import CloudCompiler (may be compiled .pyd)
    try:
    ...
    # v2.76.0: Local pre-validation before sending to cloud
    # Parse YAML+BSL locally to catch errors early with detailed messages
    ...
    # Run ProcessorValidator for detailed error reporting
    try:
    ...
    # v2.76.0: Generate local XML so Module.bsl and XML files stay up-to-date
    ...
    if errors:
        ...
        for err in errors:
            # Try to parse structured error (JSON object with module/line/message)
...
def cmd_yaml(args):
    """Команда: створити обробку з YAML конфігурації"""
    if not args.config.exists():
        print(f"❌ Помилка: Файл не знайдено: {args.config}")
        sys.exit(1)

    # v2.75.0: Cloud compilation - send YAML+BSL to cloud, no local XML generation
...
def cmd_sync(args):
    """Команда: синхронізація змін з відредагованого EPF/XML назад в YAML+BSL"""
...
def cmd_decompile(args):
    """Команда: декомпіляція EPF → XML (PRO feature since v2.51.0)"""
    if not args.epf_file.exists():
        print(f"❌ Помилка: EPF файл не знайдено: {args.epf_file}")
        sys.exit(1)

    print(f"🔄 Декомпіляція EPF → XML: {args.epf_file}")

    # v2.51.0: Use LicensedEPFCompiler (decompilation is now a PRO feature)
    ...
    if compiler.decompile_epf(args.epf_file, args.output):
        ...
        print("\n❌ Помилка декомпіляції EPF")
        sys.exit(1)


def cmd_features(args):
    """Команда: показати доступні можливості генератора (v2.44.0+)"""
    import json
...
def print_overview(registry: dict):
    """Print feature registry overview"""
...
def print_category(registry: dict, category: str):
    ...
    if isinstance(items, list) and items:
        # Check if it's common_mistakes format (wrong/correct) or standard (name/description)
        if "wrong" in items[0]:
        ...
        # For pictures/keywords which have count but not full items
...
def print_feature_details(registry: dict, name: str):
    """Print details for a specific feature"""
...
def search_features(registry: dict, query: str) -> dict:
    """Search features by query in name or description"""
    results = {}
...
# =============================================================================
# License commands (PRO monetization)
# =============================================================================

def cmd_activate(args):
    """Команда: активувати PRO ліцензію"""
...
def cmd_license_status(args):
    """Команда: показати статус ліцензії"""
    mgr = get_license_manager()
    ...
    if status.is_licensed:
        ...
        print()
        print("   Community Edition - всі локальні фічі доступні")
        print("   Cloud compilation потребує PRO ліцензію")
        print()
        print("   Cloud compilation available with PRO license")
...
def cmd_setup_1c(args):
    """Setup 1C configuration."""
    from .pro.conf_cfg_manager import run_setup_command
    return run_setup_command(check=args.check, dry_run=args.dry_run)


def cmd_clear_cache(args):
    """
    v2.66.1: Команда очищення persistent IB кешу.

    Використовується для:
    - Діагностики стану кешу (--info)
    - Примусового очищення при проблемах
    - Очищення при зміні платформи (автоматично, але можна вручну)
    """
    from .pro.persistent_ib_impl import PersistentIBManager
    from .pro.constants import EPF_COMPILER_PERSISTENT_IB

    # Створюємо менеджер з дефолтними параметрами
    manager = PersistentIBManager(designer_path=None)
    cache_info = manager.get_cache_info()

    # Режим показу інформації
    if args.info:
        if cache_info:
...
def cmd_excel2mxl(args):
    """Convert Excel file to MXL format for 1C print forms."""
    try:
...
def cmd_trial(args):
    """Команда: запросити 7-денну пробну ліцензію"""
...
def _adjust_platform_version_for_epf(args, processor):
    ...
    try:
        from .pro.epf_version_helper import adjust_platform_version
...
def generate_processor(args, processor):
    """Генерує процесор (спільна логіка для minimal, example, yaml)"""
    # Визначаємо output директорію
    output_dir = args.output or Path.cwd() / "tmp"

    if args.dry_run:
        print(f"🔍 Dry run режим - файли не будуть створені")

    # v2.64.1: При EPF генерації автоматично визначаємо версію формату XML
    # на основі встановленої платформи 1C
...
def has_validation_config(processor):
    """
    Перевіряє чи процесор має налаштування валідації (non-default).

    Якщо є будь-які налаштування валідації (syntax_check_enabled або semantic_check_enabled),
    то потрібно використовувати Configuration mode навіть без метаданих.

    Args:
        processor: Processor об'єкт

    Returns:
        True якщо є налаштування валідації, False інакше
    """
    # Перевіряємо чи синтаксична або семантична валідація увімкнені
...
def compile_to_epf(args, processor, processor_root, output_dir, generator):
    """Компілює XML в EPF формат"""
    print("\n🔧 Генерація вихідного файлу...")

    # Use LicensedEPFCompiler which checks PRO license before compilation
    compiler = LicensedEPFCompiler(args.compiler_path, use_persistent_ib=not args.no_persistent_ib)
    ...
    # Автоматично визначаємо чи потрібна Configuration
    requirements = MetadataAnalyzer.analyze_processor(processor)
    needs_validation = has_validation_config(processor)

    # Вибираємо режим компіляції
    # Configuration mode потрібен якщо:
    # 1. Є метадані (CatalogRef/DocumentRef)
    # 2. Налаштована валідація (syntax_check_enabled або semantic_check_enabled)
    ...
    if compilation_success:
        print(f"\n🎉 Готово! EPF створено: {epf_path}")
        print(f"📊 Розмір: {epf_path.stat().st_size:,} bytes")

        # v2.32.0: Save snapshot from EPF export (includes Form.xml files)
        # This fixes sync tool Form.xml detection issue
        print(f"\n📸 Створюю snapshot з EPF експорту...")
        if generator.save_snapshot_from_epf(epf_path, output_dir, compiler):
            ...
            print(f"⚠️  Помилка збереження snapshot (але EPF створено успішно)")

        # Генерація тестів якщо є tests_config (v2.16.0+)
        if processor.tests_config:
            generate_tests(processor, epf_path, output_dir, args.compiler_path, compiler.last_temp_ib)
    else:
        print("\n❌ Помилка компіляції EPF")
...
def generate_tests(processor, epf_path, output_dir, compiler_path=None, temp_ib_path=None):
    """Генерує pytest тести якщо є tests_config (v2.16.0+)"""
    ...
    try:
        # Визначаємо tests директорію
        tests_dir = output_dir / processor.name / "tests"

        # Використовуємо temp_ib з компіляції (обробка вже в конфігурації!)
        if temp_ib_path and temp_ib_path.exists():
            ...
            # Fallback - створюємо persistent_ib (старий підхід)
        ...
        if test_gen.generate():
            ...
            # v2.23.0: Інжектуємо procedural tests в ObjectModule для test EPF
            if processor.tests_config.procedural_tests:
                print("\n💉 Інжектування procedural tests в ObjectModule...")

                # Шляхи для injection
                processor_xml_dir = output_dir / processor.name  # ProcessorName/
                clean_objectmodule = processor_xml_dir / processor.name / "Ext" / "ObjectModule.bsl"

                # Створюємо validation директорію для test EPF
                # NOTE: Copy the entire processor_xml_dir (ProcessorName/) to (ProcessorName_Validation/)
                # Structure after copy:
                #   output_dir/ProcessorName_Validation/
                #     ├── ProcessorName/      (inner directory with code)
                #     └── ProcessorName.xml   (will be renamed to ProcessorName_Validation.xml)
                validation_processor_name = f"{processor.name}_Validation"
                validation_xml_dir = output_dir / validation_processor_name

                # Копіюємо ВСЮ структуру для test compilation
                import shutil
                ...
                if validation_xml_dir.exists():
                    ...
                    # Rename inner processor directory (ProcessorName/ → ProcessorName_Validation/)
                    ...
                    # Update ALL XML files to use new processor name (main XML + all form XMLs)
                    xml_files_updated = 0
                    for xml_file in validation_xml_dir.rglob("*.xml"):
                        xml_content = xml_file.read_text(encoding="utf-8")
                        original_content = xml_content

                        # Replace all processor name references
                    ...
                    # Rename the main XML file to match validation processor name
                ...
                # Injection в validation копію (use new inner directory name)
                ...
                if test_gen.inject_procedural_tests_into_objectmodule(
                    ...
                    # Компілюємо test EPF (ProcessorName_Tests.epf)
                    test_epf_path = epf_path.parent / f"{epf_path.stem}_Tests.epf"
                    print(f"\n🔧 Компіляція test EPF: {test_epf_path.name}...")

                    # Знаходимо компілятор (якщо ще не знайдено)
                    if not compiler_path:
                        compiler_path = LicensedEPFCompiler.find_platform()
                    ...
                    if compiler_path:
                        try:
                            ...
                            # v2.23.0: ALWAYS use fast mode for test EPF compilation
                            # Validation already passed for clean EPF, no need to re-validate
                            # Fast mode avoids temp_ib conflicts
                            # Structure: validation_xml_dir/validation_processor_name.xml (NOT validation_xml_dir/validation_processor_name/validation_processor_name.xml)
                            ...
                            print(f"   ⚡ Using fast mode for test EPF (validation already done for clean EPF)...")

                            test_success = test_compiler.compile_epf(test_xml_root, test_epf_path)

                            if test_success:
                                print(f"   ✅ Test EPF створено: {test_epf_path}")
                                ...
                                print(f"   ⚠️  Помилка компіляції test EPF")
                                print(f"   📁 Validation XML збережено: {validation_xml_dir}")
                        except Exception as test_compile_error:
                            print(f"   ❌ Exception під час компіляції test EPF: {test_compile_error}")
                            import traceback
                            traceback.print_exc()
                    else:
...
def create_parser():
    """Створює argparse parser з усіма командами та опціями"""
...
Приклади:
  ...
  python -m 1c_processor_generator sync --modified-xml Modified.xml --config config.yaml --handlers handlers.bsl --llm-mode
    ...
    # activate command (PRO license activation)
    ...
    # license-status command (show license status)
...
def main():
    ...
    # v2.62.0: Check for updates (background, non-blocking)
    check_version_in_background()

    parser = create_parser()
    args = parser.parse_args()

    # Викликаємо відповідну команду
