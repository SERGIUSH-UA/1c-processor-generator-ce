"""
Assertion Helper - BSL code generation + extended assertion checking (v2.16.0+ / v2.19.0+)

v2.16.0+: BSL test infrastructure generation
v2.19.0+: Extended assertion checking (numeric, string, type, collection)
"""
...
# BSL код для тестової інфраструктури
...
def get_test_infrastructure_bsl() -> str:
    """
    Повертає BSL код тестової інфраструктури.

    Returns:
        BSL код для додавання в #Область ТестоваяИнфраструктура
    """
...
def should_add_test_infrastructure(processor) -> bool:
    """
    Визначає чи треба додавати тестову інфраструктуру в Module.bsl.

    Args:
        processor: Processor object

    Returns:
        True якщо є tests_config
    """
    return hasattr(processor, "tests_config") and processor.tests_config is not None


# ===== Extended Assertion Checking (v2.19.0+) =====


def check_numeric_assertion(actual: Any, expected: dict, attr_name: str) -> Tuple[bool, Optional[str]]:
    """
    Check numeric assertions (v2.19.0+).

    Supported: greater_than, less_than, gte, lte, between, not_equal
...
def check_string_assertion(actual: Any, expected: dict, attr_name: str) -> Tuple[bool, Optional[str]]:
    """
    Check string assertions (v2.19.0+).

    Supported: matches, starts_with, ends_with, contains, length
...
def check_type_assertion(actual: Any, expected: dict, attr_name: str) -> Tuple[bool, Optional[str]]:
    """
    Check type assertions (v2.19.0+).

    Supported: type (string/number/date/boolean), is_null, not_null
...
def check_collection_assertion(actual: Any, expected: dict, attr_name: str) -> Tuple[bool, Optional[str]]:
    """
    Check collection assertions (v2.19.0+).
...
def check_extended_assertion(actual: Any, expected: Any, attr_name: str) -> Tuple[bool, Optional[str]]:
    """
    Main function to check extended assertions (v2.19.0+).

    Auto-detects assertion type:
    - Simple value: equality check
    - Dict with assertions: extended checks

    Args:
        actual: Actual value
        expected: Expected (simple or dict with assertions)
    ...
    # Check if dict has assertion keys
    ...
    if not any(key in expected for key in all_keys):
        # Not an assertion dict, simple equality
        if actual != expected:
            return False, f"{attr_name}: expected {expected}, got {actual}"
        return True, None
...
def check_message_assertion_extended(messages: List[str], assertion: Any) -> Tuple[bool, Optional[str]]:
    """
    Check extended message assertions (v2.19.0+).

    Supports MessageAssertion with: contains, equals, count, matches, starts_with, ends_with

    Args:
        messages: List of messages
        assertion: MessageAssertion object or dict
