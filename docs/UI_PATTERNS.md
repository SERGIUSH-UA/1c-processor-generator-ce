...
> ⚠️ **Свойства ДиалогВыбораФайла — частая ошибка.**
> У объекта `ДиалогВыбораФайла` **НЕТ свойства `ИмяФайла`**. Попытка `Диалог.ИмяФайла = "..."`
> даёт ошибку выполнения: `Поле объекта не обнаружено (ИмяФайла)`.
> Для предзаполнения и чтения имени файла используйте **`ПолноеИмяФайла`** (строка),
> для каталога — `Каталог`, для множественного выбора результат — `ВыбранныеФайлы` (массив).
>
> ```bsl
> // ❌ НЕВЕРНО — свойства не существует:
> Диалог.ИмяФайла = "nomenclature_1c.json";
> // ✅ ВЕРНО:
> Диалог.ПолноеИмяФайла = "nomenclature_1c.json";
> ```

**Сохранение файла (асинхронный диалог, обязателен для 8.3.25+ с запретом модальности):**
```bsl
&НаКлиенте
Процедура СохранитьФайл(Команда)
    ...
    Диалог.ПолноеИмяФайла = "export.json";   // предзаполнение имени — НЕ ИмяФайла!

    Оповещение = Новый ОписаниеОповещения("СохранитьЗавершение", ЭтотОбъект, ДанныеСтрокой);
    Диалог.Показать(Оповещение);   // асинхронно; синхронный .Выбрать() упадёт при запрете модальности
КонецПроцедуры

&НаКлиенте
Процедура СохранитьЗавершение(ВыбранныеФайлы, ДанныеСтрокой) Экспорт
    Если ВыбранныеФайлы = Неопределено ИЛИ ВыбранныеФайлы.Количество() = 0 Тогда
        Возврат;
    КонецЕсли;
    ТекстовыйДокумент = Новый ТекстовыйДокумент;
    ТекстовыйДокумент.УстановитьТекст(ДанныеСтрокой);
    ТекстовыйДокумент.Записать(ВыбранныеФайлы[0], КодировкаТекста.UTF8);
КонецПроцедуры
```
